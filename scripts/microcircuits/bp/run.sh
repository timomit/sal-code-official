...
for sweep_file in "${SWEEP_FILES[@]}"; do
    ...
    python "$TRAIN" "$sweep_file" "$sweep_id" -o "$SCRIPT_DIR/../../../results/microcircuits/$EXP_TYPE"&
    # done
done

# --- Step 3: wait for all background jobs ---
wait
echo "All $NUM_SWEEPS runs finished."
